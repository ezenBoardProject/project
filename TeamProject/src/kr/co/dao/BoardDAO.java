...
public class BoardDAO {
	...
	public void insert(BoardDTO dto) {
		...
		String sql = "INSERT INTO bd_table (num, id, title, content, repRoot, repStep, repIndent) VALUES(?,?,?,?,?,?,?)";
	...
	private int getNum(Connection conn) {
		int num = -1;
		PreparedStatement pstmt = null;
		String sql = "SELECT NVL2(MAX(num), MAX(num)+1, 1) FROM bd_table";
		ResultSet rs = null;
	...
	private void increaseReadcnt(Connection conn, int num) {
		PreparedStatement pstmt = null;
		String sql = "UPDATE bd_table SET readcnt = readcnt + 1 WHERE num = ?";
	...
	public BoardDTO read(int num) {
		...
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "SELECT * FROM bd_table WHERE num = ?";
		ResultSet rs = null;
	...
	public BoardDTO updateui(int num) {
		...
		try {
			...
			if(rs.next()) {
				...
				dto= new BoardDTO(num, id, title, content, 
	...
	public void update(BoardDTO dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "UPDATE bd_table SET title = ?, id = ?, "
	...
	public void delete(int num) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "DELETE FROM bd_table WHERE num = ?";
	...
	public List<BoardDTO> search(String searchOption, String searchKeyword) {
		...
		String sql = "SELECT * FROM bd_table WHERE UPPER("+searchOption+") LIKE UPPER(?) ORDER BY repRoot DESC, repStep ASC";
		...
		try {
			...
			while (rs.next()) {
				...
				BoardDTO dto = new BoardDTO(num, id, title, 
	...
	public void reply(int oriNum, BoardDTO repDTO) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		String sql = "INSERT INTO bd_table "
	...
	private void increaseRepStepOtherReply(Connection conn, BoardDTO oriDTO) {
		PreparedStatement pstmt = null;
		String sql = "UPDATE bd_table SET repStep = repStep + 1 "
	...
	public PageTO page(int curPage) {
		...
		String sql = "SELECT * FROM "
				+ "(SELECT ROWNUM rnum, num, title, id, writeday, readcnt, repIndent FROM "
				+ "(SELECT * FROM bd_table order by repRoot desc, repStep asc)) "
				+ "WHERE rnum >= ? AND rnum <= ?";
		ResultSet rs = null;
	...
	private int getAmount(Connection conn) {
		int amount =0;
		PreparedStatement pstmt = null;
		String sql = "select count(num) from bd_table";
		ResultSet rs = null;
		try {
			pstmt = conn.prepareStatement(sql);
			rs= pstmt.executeQuery();
